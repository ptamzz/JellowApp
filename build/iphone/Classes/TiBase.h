...
// in simulator we redefine to format for Jellow Developer console
...
{\
	NSString * exceptionName = [@"org.jellow." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.jellow";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	TiExceptionThrowWithNameAndReason(exceptionName,message);\
}\
...
enum {
	UIJellowNativeItemNone = -1, 
	UIJellowNativeItemSpinner = -2,
	UIJellowNativeItemProgressBar = -3,
	
	UIJellowNativeItemSlider = -4,
	UIJellowNativeItemSwitch = -5,
	UIJellowNativeItemMultiButton = -6,
	UIJellowNativeItemSegmented = -7,
	
	UIJellowNativeItemTextView = -8,
	UIJellowNativeItemTextField = -9,
	UIJellowNativeItemSearchBar = -10,
	
	UIJellowNativeItemPicker = -11,
	UIJellowNativeItemDatePicker = -12,
	
	UIJellowNativeItemInfoLight = -13,
	UIJellowNativeItemInfoDark = -14,
	
	UIJellowNativeItemDisclosure = -15,
	
	UIJellowNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all Jellow instances of
